#!/bin/bash
# TWGCB-01-008-0109: All interfaces must NOT send ICMP redirects (RHEL 8.5)
# Requirement: net.ipv4.conf.all.send_redirects = 0 (persisted) and runtime value = 0
...
TITLE="TWGCB-01-008-0109: Disable ICMP send_redirects on all interfaces (send_redirects=0)"
GREEN="\e[1;92m"   # bright green
RED="\e[1;91m"     # bright red
YELLOW="\e[1;33m"
RESET="\e[0m"

PERSIST_FILE="/etc/sysctl.d/99-twgcb-0109.conf"
...
check_compliance() {
  echo "Checking ICMP send_redirects (all interfaces) ..."
  echo "Check results:"
  show_check_lines
