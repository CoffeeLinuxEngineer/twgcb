#!/bin/bash
# TWGCB-01-008-0170: Ensure auditd records kmod command usage
# Target: RHEL 8.5
...
echo -e "TWGCB-01-008-0170: Ensure auditd records kmod command usage\n"
...
if check_compliance; then
    echo -e "${C_GREEN}Compliant:${C_OFF} kmod command usage is being recorded by auditd."
    exit 0
else
    echo -e "${C_RED}Non-compliant:${C_OFF} kmod audit rule is missing."
fi

# ---------- Prompt ----------
while true; do
    echo -n "Apply fix now (append audit rule for /bin/kmod and reload)? [Y]es / [N]o / [C]ancel: "
    read -rn1 ans
    echo
    case "$ans" in
        Y|y) break ;;
