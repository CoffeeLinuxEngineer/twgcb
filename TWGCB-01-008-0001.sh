...
# Notes:
#   * No Chinese in code (per user preference)
...
show_checks() {
  ...
  # Show the exact modprobe simulation output (single line prefixed with 'Line: ')
  ...
  if [[ -f "$RULE_FILE" ]]; then
    ...
    # Show matching/non-matching with line numbers
...
apply_fix() {
  ...
  if lsmod | awk '{print $1}' | grep -qx "cramfs"; then
    if ! rmmod cramfs 2>/dev/null; then
      # If removal fails (e.g., in use), report but continue (the blacklist still prevents future loads)
