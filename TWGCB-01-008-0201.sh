#!/bin/bash
# TWGCB-01-008-0201: Ensure /etc/cron.monthly directory ownership
# Target OS: RHEL 8.5
...
echo "TWGCB-01-008-0201: Ensure /etc/cron.monthly directory ownership"
...
if check_compliance; then
    echo -e "${CLR_GREEN}Compliant: /etc/cron.monthly owned by root:root.${CLR_RESET}"
    exit 0
else
    echo -e "${CLR_RED}Non-compliant: /etc/cron.monthly ownership incorrect or directory missing.${CLR_RESET}"
fi
