#!/bin/bash
# TWGCB-01-008-0163: Ensure auditd records ssh-agent usage
# Target: RHEL 8.5
# Required rule (UID_MIN dynamic):
#   -a always,exit -F path=/usr/bin/ssh-agent -F perm=x -F auid>=UID_MIN -F auid!=4294967295 -k privileged-ssh
...
# Determine UID_MIN dynamically; default to 1000 if not found
UID_MIN=$(awk '/^\s*UID_MIN/{print $2}' /etc/login.defs)
[[ -z "$UID_MIN" ]] && UID_MIN=1000

RULE="-a always,exit -F path=/usr/bin/ssh-agent -F perm=x -F auid>=$UID_MIN -F auid!=4294967295 -k privileged-ssh"

echo -e "TWGCB-01-008-0163: Ensure auditd records ssh-agent usage\n"
...
show_status() {
    ...
    if [[ -f "$AUDIT_FILE" ]]; then
        if grep -qF -- "$RULE" "$AUDIT_FILE"; then
            line=$(grep -nF -- "$RULE" "$AUDIT_FILE" | head -n1 | cut -d: -f1)
            echo "Line: ${line:-?}: $RULE"
        else
            echo "(Missing) $RULE"
        fi
...
if check_compliance; then
    echo -e "${C_GREEN}Compliant:${C_OFF} ssh-agent usage is being recorded by auditd."
    exit 0
else
    echo -e "${C_RED}Non-compliant:${C_OFF} ssh-agent audit rule is missing."
fi

# ---------- Prompt ----------
while true; do
    echo -n "Apply fix now (append audit rule for ssh-agent and reload)? [Y]es / [N]o / [C]ancel: "
    read -rn1 ans
    echo
    case "$ans" in
        Y|y) break ;;
