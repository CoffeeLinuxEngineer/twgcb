...
show_matches() {
  ...
  local i=0
  # Correct field parsing for /etc/passwd: name:passwd:uid:gid:gecos:home:shell
  while IFS=: read -r name passwd uid gid gecos home shell; do
    # Only list real users (uid >= 1000) and exclude nfsnobody
    if [ -n "$uid" ] && [ "$uid" -ge 1000 ] 2>/dev/null && [ "$name" != "nfsnobody" ]; then
      i=$((i+1))
...
check_compliance() {
  ...
  # 1: non-compliant
  # 2: cannot verify
  check_defaults_compliant; local d_rc=$?
  check_users_compliant; local u_rc=$?
