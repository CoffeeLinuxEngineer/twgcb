#!/bin/bash
# TWGCB-01-008-0164: Ensure auditd records unix_update helper usage
# Target: RHEL 8.5
# Required rule (UID_MIN dynamic):
#   -a always,exit -F path=/sbin/unix_update -F perm=x -F auid>=UID_MIN -F auid!=4294967295 -k privileged-unix-update
...
RULE="-a always,exit -F path=/sbin/unix_update -F perm=x -F auid>=$UID_MIN -F auid!=4294967295 -k privileged-unix-update"

echo -e "TWGCB-01-008-0164: Ensure auditd records unix_update helper usage\n"
...
if check_compliance; then
    echo -e "${C_GREEN}Compliant:${C_OFF} unix_update usage is being recorded by auditd."
    exit 0
else
    echo -e "${C_RED}Non-compliant:${C_OFF} unix_update audit rule is missing."
fi

# ---------- Prompt ----------
while true; do
    echo -n "Apply fix now (append audit rule for unix_update and reload)? [Y]es / [N]o / [C]ancel: "
    read -rn1 ans
    echo
    case "$ans" in
        Y|y) break ;;
