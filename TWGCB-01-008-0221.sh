...
# Platform: RHEL 8.5
# Policy for RHEL 8.2+:
#   - /etc/security/faillock.conf must have an active line: unlock_time = 900 (or higher)
#   - PAM must include pam_faillock.so (feature typically via authselect with-faillock)
# Notes:
#   - Uses bright green/red messages, shows "Line: N:" with file paths, prompts [Y]es / [N]o / [C]ancel.
#   - Applies by setting faillock.conf and enabling with-faillock via authselect (fallback to editing PAM files directly).
...
get_active_unlock_time() {
  # Echo the last active unlock_time numeric value from faillock.conf, or empty if none/unreadable
  [ ! -r "$FAILLOCK" ] && return 1
  awk '
    {
      s=$0
...
pam_faillock_present() {
  # 0 if at least one PAM file has an active pam_faillock.so line; else 1; 2 if unreadable
  local any_perm=0
  local present=1
  for f in "${PAM_FILES[@]}"; do
    if [ ! -r "$f" ]; then
...
check_compliance() {
  # Returns:
  # 0: compliant (unlock_time >= REQUIRED_UNLOCK and pam_faillock present)
  # 1: non-compliant
  # 2: cannot verify
  local v
  ...
  if ! [ "$v" -ge "$REQUIRED_UNLOCK" ] 2>/dev/null; then
    return 1
  fi
...
ensure_unlock_time_in_failock() {
  # Set unlock_time to REQUIRED_UNLOCK in FAILLOCK
  if [ ! -e "$FAILLOCK" ]; then
    # Create with minimal content
    {
      echo "# Managed by TWGCB-01-008-0221"
      echo "unlock_time = ${REQUIRED_UNLOCK}"
    } > "$FAILLOCK" 2>/dev/null || return 1
    return 0
  fi
  if [ ! -w "$FAILLOCK" ]; then
    return 1
...
ensure_pam_faillock_in_file() {
  # Insert basic pam_faillock lines if missing (minimal safe set)
  local file="$1"
  [ ! -w "$file" ] && return 1
...
apply_fix() {
  ...
  # 1) Ensure unlock_time in faillock.conf
  ensure_unlock_time_in_failock || ok=1

  # 2) Ensure pam_faillock is active
  local rc
  enable_faillock_with_authselect
  rc=$?
  if [ $rc -eq 2 ]; then
    # No authselect; edit PAM files directly
    local failed=0
    for f in "${PAM_FILES[@]}"; do
