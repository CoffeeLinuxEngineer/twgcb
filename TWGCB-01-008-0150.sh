#!/bin/bash
# TWGCB-01-008-0150: Audit session start information (RHEL 8.5)
# Monitors changes to utmp/wtmp/btmp to record logins, logouts, reboots, and failed logins.
# Required watch rules (baseline):
#   -w /var/run/utmp -p wa -k session
#   -w /var/log/wtmp -p wa -k logins
#   -w /var/log/btmp -p wa -k logins
# Behavior:
#   - Checks /etc/audit/rules.d/audit.rules for the three watch rules
...
ITEM_ID="TWGCB-01-008-0150"
TITLE="Record session start information (audit rules)"
...
# Tolerant regexes: -w <path> ... -p containing w and a ... -k session/logins
REQ_REGEX=(
  '^[[:space:]]*-w[[:space:]]+/var/run/utmp([[:space:]]+.*)?-p[[:space:]]*[rwxad]*w[rwxad]*a[rwxad]*([[:space:]]+.*)?-k[[:space:]]*session([[:space:]]|$)'
  '^[[:space:]]*-w[[:space:]]+/var/log/wtmp([[:space:]]+.*)?-p[[:space:]]*[rwxad]*w[rwxad]*a[rwxad]*([[:space:]]+.*)?-k[[:space:]]*logins([[:space:]]|$)'
  '^[[:space:]]*-w[[:space:]]+/var/log/btmp([[:space:]]+.*)?-p[[:space:]]*[rwxad]*w[rwxad]*a[rwxad]*([[:space:]]+.*)?-k[[:space:]]*logins([[:space:]]|$)'
)
...
if [[ ${#missing_idx[@]} -eq 0 ]]; then
  echo -e "${GREEN}Compliant:${RESET} All required session/logins watch rules are present."
  exit 0
fi
...
{
  echo
  echo "# ${ITEM_ID} — audit session start information"
  for idx in "${missing_idx[@]}"; do
    echo "${REQ_LINES[$idx]}"
  done
