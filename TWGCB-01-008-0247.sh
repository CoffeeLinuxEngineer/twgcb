#!/bin/bash
# TWGCB-01-008-0247
# When using firewalld, disable and mask the nftables service.
...
if check_compliance; then
    echo -e "${GREEN}Compliant: nftables is masked and not active (or unit not present).${RESET}"
    exit 0
else
    echo -e "${RED}Non-compliant: nftables is not masked and stopped.${RESET}"
fi

while true; do
    echo -n "Apply fix now (run 'systemctl --now mask nftables')? [Y]es / [N]o / [C]ancel: "
    read -rsn1 key
    echo
    case "$key" in
        [Yy])
