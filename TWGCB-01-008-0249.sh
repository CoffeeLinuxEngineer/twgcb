...
check_compliance() {
    # Compliant if the service exists, is enabled, and is active
...
apply_fix() {
    # Enable and start the service now (idempotent)
...
# --- Main ---
echo "TWGCB-01-008-0249: Enable and start nftables"
echo
...
if check_compliance; then
    echo -e "${GREEN}Compliant: nftables is enabled and active.${RESET}"
    exit 0
else
    echo -e "${RED}Non-compliant: nftables is not enabled and active.${RESET}"
fi

while true; do
    echo -n "Apply fix now (run 'systemctl --now enable nftables')? [Y]es / [N]o / [C]ancel: "
    read -rsn1 key
    echo
    case "$key" in
        [Yy])
