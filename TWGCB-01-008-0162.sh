#!/bin/bash
# TWGCB-01-008-0162: Ensure auditd records chcon command usage
# Target: RHEL 8.5
# Required rule (UID_MIN dynamic):
#   -a always,exit -F path=/usr/bin/chcon -F perm=x -F auid>=UID_MIN -F auid!=4294967295 -k perm_chng
...
RULE="-a always,exit -F path=/usr/bin/chcon -F perm=x -F auid>=$UID_MIN -F auid!=4294967295 -k perm_chng"

echo -e "TWGCB-01-008-0162: Ensure auditd records chcon command usage\n"
...
if check_compliance; then
    echo -e "${C_GREEN}Compliant:${C_OFF} chcon command usage is being recorded by auditd."
    exit 0
else
    echo -e "${C_RED}Non-compliant:${C_OFF} chcon audit rule is missing."
fi

# ---------- Prompt ----------
while true; do
    echo -n "Apply fix now (append audit rule for chcon and reload)? [Y]es / [N]o / [C]ancel: "
    read -rn1 ans
    echo
    case "$ans" in
        Y|y) break ;;
