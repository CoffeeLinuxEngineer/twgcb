#!/bin/bash
#
# TWGCB-01-008-0219: Prohibit dictionary words as passwords
# Target OS: Red Hat Enterprise Linux 8.5
# This script checks and enforces dictcheck=1 in /etc/security/pwquality.conf
...
echo "TWGCB-01-008-0219: Prohibit dictionary words as passwords"
echo "Checking file: $TARGET"
echo "Check results:"

show_lines() {
    # Display matching config lines with numbered prefix. Requires readable file.
    grep -n -E "^[[:space:]]*${SETTING_KEY}[[:space:]]*=" "$TARGET" 2>/dev/null | sed 's/^[0-9][0-9]*/Line: &:/'
}

check_compliance() {
    # Return 0 if compliant, 1 otherwise.
    # Compliant iff file exists, is readable, and contains dictcheck=1 (ignoring trailing comments and whitespace)
    [ ! -r "$TARGET" ] && return 1
...
if check_compliance; then
    echo -e "${GREEN}Compliant: dictcheck is set to ${REQUIRED_VALUE}.${RESET}"
    exit 0
else
    echo -e "${RED}Non-compliant: dictcheck is missing or not set to ${REQUIRED_VALUE}.${RESET}"
fi
...
while true; do
    ...
    case "$key" in
        [Yy])
            # Apply only if the target exists and is writable
            if [ ! -e "$TARGET" ]; then
            ...
            # Update existing line or append if absent
