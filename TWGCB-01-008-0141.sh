#!/bin/bash
# TWGCB-01-008-0141: Ensure /etc/audit/rules.d/audit.rules permission is 600 or stricter
# Platform: Red Hat Enterprise Linux 8.5
# Behavior:
#   - Checks /etc/audit/rules.d/audit.rules for mode <= 600.
...
TITLE="TWGCB-01-008-0141: Ensure /etc/audit/rules.d/audit.rules permission is 600 or stricter"
...
prompt_apply() {
  ...
  while true; do
    echo -ne "Apply fix now (set chmod 600 on $TARGET; skip missing=${skip_missing_choice})? [Y]es / [N]o / [C]ancel: "
    IFS= read -rsn1 ans || true
    echo
    case "${ans^^}" in
      Y|"")
