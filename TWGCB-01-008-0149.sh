#!/bin/bash
# TWGCB-01-008-0149: Audit changes to login/logout info (RHEL 8.5)
# Monitors changes to:
#   - /var/run/faillock/ (failed login records)
#   - /var/log/lastlog  (last successful login per user)
# Required watch rules (baseline):
#   -w /var/run/faillock/ -p wa -k logins
#   -w /var/log/lastlog   -p wa -k logins
# Behavior:
#   - Checks /etc/audit/rules.d/audit.rules for both watches
...
ITEM_ID="TWGCB-01-008-0149"
TITLE="Record changes to login/logout info (audit rules)"
...
# Tolerant regexes: -w <path> ... -p containing w and a ... -k logins
REQ_REGEX=(
  '^[[:space:]]*-w[[:space:]]+/var/run/faillock/([[:space:]]+.*)?-p[[:space:]]*[rwxad]*w[rwxad]*a[rwxad]*([[:space:]]+.*)?-k[[:space:]]*logins([[:space:]]|$)'
  '^[[:space:]]*-w[[:space:]]+/var/log/lastlog([[:space:]]+.*)?-p[[:space:]]*[rwxad]*w[rwxad]*a[rwxad]*([[:space:]]+.*)?-k[[:space:]]*logins([[:space:]]|$)'
)
...
if [[ ${#missing_idx[@]} -eq 0 ]]; then
  echo -e "${GREEN}Compliant:${RESET} Both login/logins watch rules are present."
  exit 0
fi
...
{
  echo
  echo "# ${ITEM_ID} — audit changes to login/logout info"
  for idx in "${missing_idx[@]}"; do
    echo "${REQ_LINES[$idx]}"
  done
