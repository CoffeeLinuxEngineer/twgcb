#!/bin/bash
# TWGCB-01-008-0168: Ensure auditd records usermod command usage
# Target: RHEL 8.5
# Requirement per policy: -a always,exit -F path=/usr/sbin/usermod -F perm=x -F auid>=UID_MIN -F auid!=4294967295 -k privileged-usermod
...
RULE="-a always,exit -F path=/usr/sbin/usermod -F perm=x -F auid>=$UID_MIN -F auid!=4294967295 -k privileged-usermod"

echo -e "TWGCB-01-008-0168: Ensure auditd records usermod command usage\n"
...
if check_compliance; then
    echo -e "${C_GREEN}Compliant:${C_OFF} usermod command usage is being recorded by auditd."
    exit 0
else
    echo -e "${C_RED}Non-compliant:${C_OFF} usermod audit rule is missing."
fi

# ---------- Prompt ----------
while true; do
    echo -n "Apply fix now (append audit rule for usermod and reload)? [Y]es / [N]o / [C]ancel: "
    read -rn1 ans
    echo
    case "$ans" in
        Y|y) break ;;
