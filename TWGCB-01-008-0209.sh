...
#                   then modify its files. Use --force to allow switching if local changes exist.
            ...
            if authselect_available; then
                ...
                if [[ -n "$PROFILE_ID" && "$PROFILE_ID" == custom/* ]]; then
                    ...
                    if [ ! -d "/etc/authselect/custom/twgcb" ]; then
                        if ! authselect create-profile twgcb -b "$BASE" --symlink-meta; then
                            echo -e "${RED}Failed to create custom profile based on '$BASE'.${RESET}"
                            APPLY_FAIL=1
                        fi
                    fi
                    if [ $APPLY_FAIL -eq 0 ]; then
                        # Split FEATURES into array to preserve words
                        read -r -a FEATS_ARR <<< "$FEATURES"
                        echo "Selecting profile: custom/twgcb ${FEATURES}"
                        if ! authselect select custom/twgcb "${FEATS_ARR[@]}" --force; then
                            echo -e "${RED}Failed to select custom/twgcb profile (try with --force).${RESET}"
                            APPLY_FAIL=1
                        fi
                    fi
