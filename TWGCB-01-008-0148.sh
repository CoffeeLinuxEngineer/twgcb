#!/bin/bash
# TWGCB-01-008-0148: Audit sudoers file/directory changes (RHEL 8.5)
# Baseline watch rules:
...
ITEM_ID="TWGCB-01-008-0148"
TITLE="Record system administrator activity (sudoers watch)"
...
# Tolerant regexes: -w <path> ... -p containing w and a ... -k scope
REQ_REGEX=(
  '^[[:space:]]*-w[[:space:]]+/etc/sudoers([[:space:]]+.*)?-p[[:space:]]*[rwxad]*w[rwxad]*a[rwxad]*([[:space:]]+.*)?-k[[:space:]]*scope([[:space:]]|$)'
  '^[[:space:]]*-w[[:space:]]+/etc/sudoers\.d/([[:space:]]+.*)?-p[[:space:]]*[rwxad]*w[rwxad]*a[rwxad]*([[:space:]]+.*)?-k[[:space:]]*scope([[:space:]]|$)'
)
...
if [[ ${#missing_idx[@]} -eq 0 ]]; then
  echo -e "${GREEN}Compliant:${RESET} Both sudoers watch rules are present."
  exit 0
fi
...
{
  echo
  echo "# ${ITEM_ID} — audit sudoers changes"
  for idx in "${missing_idx[@]}"; do
    echo "${REQ_LINES[$idx]}"
  done
