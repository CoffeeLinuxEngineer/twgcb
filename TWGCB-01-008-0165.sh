#!/bin/bash
# TWGCB-01-008-0165: Ensure auditd records setfacl command usage
# Target: RHEL 8.5
# Requirement: -a always,exit -F path=/usr/bin/setfacl -F perm=x -F auid>=UID_MIN -F auid!=4294967295 -k perm_chng
...
RULE="-a always,exit -F path=/usr/bin/setfacl -F perm=x -F auid>=$UID_MIN -F auid!=4294967295 -k perm_chng"

echo -e "TWGCB-01-008-0165: Ensure auditd records setfacl command usage\n"
...
if check_compliance; then
    echo -e "${C_GREEN}Compliant:${C_OFF} setfacl command usage is being recorded by auditd."
    exit 0
else
    echo -e "${C_RED}Non-compliant:${C_OFF} setfacl audit rule is missing."
fi

# ---------- Prompt ----------
while true; do
    echo -n "Apply fix now (append audit rule for setfacl and reload)? [Y]es / [N]o / [C]ancel: "
    read -rn1 ans
    echo
    case "$ans" in
        Y|y) break ;;
