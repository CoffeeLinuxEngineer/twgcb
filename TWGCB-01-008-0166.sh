#!/bin/bash
# TWGCB-01-008-0166: Ensure auditd records finit_module system call usage
# Target: RHEL 8.5
# Rules required (adjust UID_MIN dynamically):
#   -a always,exit -F arch=b32 -S finit_module -F auid>=UID_MIN -F auid!=4294967295 -k module_chng
#   -a always,exit -F arch=b64 -S finit_module -F auid>=UID_MIN -F auid!=4294967295 -k module_chng
...
echo -e "TWGCB-01-008-0166: Ensure auditd records finit_module system call usage\n"
...
if check_compliance; then
    echo -e "${C_GREEN}Compliant:${C_OFF} finit_module audit rules are configured."
    exit 0
else
    echo -e "${C_RED}Non-compliant:${C_OFF} one or more finit_module audit rules are missing."
fi
