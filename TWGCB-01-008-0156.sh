...
ITEM_ID="TWGCB-01-008-0156"
TITLE="Record changes to identity files (audit rules)"
...
# Tolerant regexes: -w <path> ... -p containing w and a ... -k identity
REQ_REGEX=(
...
if [[ ${#missing_idx[@]} -eq 0 ]]; then
  echo -e "${GREEN}Compliant:${RESET} All identity file watch rules are present."
  exit 0
fi
...
{
  echo
  echo "# ${ITEM_ID} — audit identity file changes"
  for idx in "${missing_idx[@]}"; do
    echo "${REQ_LINES[$idx]}"
  done
