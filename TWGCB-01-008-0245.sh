...
check_compliance() {
    # Compliant if unit exists AND is enabled AND active
...
# --- Main ---
echo "TWGCB-01-008-0245: Enable and start firewalld"
echo
...
if check_compliance; then
    echo -e "${GREEN}Compliant: firewalld is enabled and active.${RESET}"
    exit 0
else
    echo -e "${RED}Non-compliant: firewalld is not enabled and active.${RESET}"
fi

while true; do
    echo -n "Apply fix now (run 'systemctl --now enable firewalld')? [Y]es / [N]o / [C]ancel: "
    read -rsn1 key
    echo
    case "$key" in
        [Yy])
