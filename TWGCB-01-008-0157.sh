#!/bin/bash
# TWGCB-01-008-0157: Audit filesystem mount events
# Target OS: RHEL 8.5
...
#   - Checks /etc/audit/rules.d/mounts.rules for both b64 and b32 rules
...
ITEM_ID="TWGCB-01-008-0157"
TITLE="Record filesystem mount events (audit rules)"
RULES_DIR="/etc/audit/rules.d"
RULES_FILE="${RULES_DIR}/mounts.rules"
...
candidate_grep() {
  ...
  local file="$1" arch="$2"
  grep -nE "^[[:space:]]*-a[[:space:]]+always,exit\b.*arch=${arch}\b.*-k[[:space:]]*mounts([[:space:]]|$)" "$file" 2>/dev/null
}

line_has_all_tokens() {
  # $1=line content
  local line="$1"
  printf '%s\n' "$line" | grep -qE '(^|[[:space:]])-S[[:space:]]+mount([[:space:]]|$)' || return 1
...
show_matching_rule() {
  # $1=file, $2=arch -> prints "Line: n: <line>" or indicates status
  local file="$1" arch="$2"
  if [[ -r "$file" ]]; then
    local found=0
...
if [[ ${#missing[@]} -eq 0 ]]; then
  echo -e "${GREEN}Compliant:${RESET} Both b64 and b32 mount rules are present."
  exit 0
fi
...
{
  echo
  echo "# ${ITEM_ID} — audit mount events (UID_MIN=${UID_MIN})"
  for m in "${missing[@]}"; do
    if [[ "$m" == "b64" ]]; then
      echo "${REQ_LINES[0]}"
    else
